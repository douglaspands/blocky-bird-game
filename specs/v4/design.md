...
# Parte VI — v4: Web via WebAssembly (pygbag)

Duas descobertas mudaram a forma de chegar nos requisitos da v4, feitas lendo a infraestrutura real do projeto antes de desenhar esta parte, não hipoteticamente:

1. **O runtime do `pygbag`** (CPython + SDL2 + pygame-ce compilados para WebAssembly, algumas dezenas de MB) **não vem embutido no build padrão** — por padrão é buscado de um CDN de terceiros (`pygame-web.github.io/archives`) a cada carregamento. Um `.html` "único" de verdade (R40) exige apontar `pygbag` para um espelho local desse runtime e depois um passo de pós-processamento que embute tudo em base64 no próprio arquivo.
2. **GitHub Pages já está em uso** por `.github/workflows/docs.yml`, publicando o site do MkDocs (seção 41). GitHub Pages via GitHub Actions só aceita uma fonte de deploy ativa por repositório — um segundo workflow concorrente sobrescreveria silenciosamente o outro a cada push. A seção 52 estende `docs.yml` em vez de criar um workflow paralelo.

## 45. Laço assíncrono (R37.2, R37.3)

**Problema.** `pygbag` exige que o laço principal seja uma corrotina que devolve o controle ao navegador periodicamente (`async def main(): ...; await asyncio.sleep(0)`, executada via `asyncio.run`) — sem isso a aba trava, porque não há uma segunda thread rodando o event loop do JavaScript por baixo.

**Solução.** `Game.run()` vira `async def`, com um único `await asyncio.sleep(0)` por iteração do laço. Em CPython nativo esse `await` é uma cessão de controle ao *event loop* do `asyncio` sem I/O real — custo desprezível a 60 FPS — então o MESMO laço roda sem ramo de plataforma tanto sob Emscripten quanto no desktop/Android nativos:

```python
async def run(self) -> None:
    """Laco principal. `await asyncio.sleep(0)` devolve o controle ao navegador a
    cada quadro (R37.2) - sob Emscripten/pygbag e o unico jeito de a aba nao travar;
    em CPython nativo o mesmo await custa uma cessao de controle sem I/O real, entao
    o MESMO laco roda sem ramo de plataforma no desktop/Android (R37.3)."""
    while self.running:
...
**Descoberto na implementação (blast radius nos testes).** `game.run()` é chamado de forma síncrona em três pontos hoje: `tests/test_game.py` (a asserção de que sair do laço grava o recorde de uma partida abandonada) e dois helpers homônimos `_run_frames`, um em `tests/test_game.py` (testes de timestep fixo, R28) e outro em `tests/test_quality.py` (testes de qualidade adaptativa, R29). Depois de `run` virar `async def`, chamar `game.run()` sem `await`/`asyncio.run` só cria uma corrotina nunca executada — **o laço não roda nenhum frame, sem lançar exceção**, e os três testes passariam vazios (falso positivo, não uma falha visível). A correção é mecânica: envolver as três chamadas em `asyncio.run(game.run())`. Nenhuma dependência nova (`pytest-asyncio`) é necessária, porque os testes em si continuam funções síncronas — só o que eles chamam passou a ser assíncrono. Esta conversão (task 84) é pré-requisito obrigatório de tudo mais nesta parte: `pygbag` não consegue rodar o laço síncrono atual.

**Risco não resolvido por leitura de código, só por teste real.** Não há confirmação de que `pygame.time.Clock.tick()` (que chama `SDL_Delay` por baixo) se comporta corretamente sob o modelo de build Asyncify do Emscripten quando intercalado com `await asyncio.sleep(0)`. Os exemplos oficiais do `pygbag` usam `clock.tick(fps)` sem modificação ao lado do `await`, o que sugere que funciona, mas é evidência indireta — a task 85 confirma isso empiricamente.

## 46. Renderizador sob Emscripten (R37.5, R40.4)

**Zero mudança de código esperada.** A cascata de três níveis de `render.py` (seção 26) já trata como casos de primeira classe tanto a falha de `import` de `pygame._sdl2.video` (`except ImportError: video = None`) quanto a falha de construção do renderer GPU (`try/except Exception` ao redor de `video.Renderer(...)`). Se o build do `pygame-ce` que o `pygbag` empacota simplesmente não compilar `_sdl2.video` para WASM, ou se a construção do renderer acelerado falhar sob Emscripten, a cascata já cai para `SurfaceRenderer` exatamente como faz hoje num Android antigo (API 21) sem driver acelerado — mesmo código, mesma linha de log, mesmo `backend` reportado pela sobreposição de diagnóstico (R30.2).

**Configuração de build exigida.** O contexto gráfico do `pygbag` é escolhido em tempo de build e não pode trocar depois de inicializado — 2D (canvas) por padrão, ou 3D/WebGL quando o app usa `pygame._sdl2`/OpenGL, exigindo uma flag de template dedicada no comando de build (R40.4).

**Risco conhecido, a ser resolvido por spike (task 85), não assumido.** Não há confirmação de que `video.Renderer(accelerated=1)` de fato constrói com sucesso sob Emscripten hoje. O pior caso já é coberto pela rede de segurança acima ("cai para superfície", não "quebra") — o que falta descobrir é só qual nível realmente vence, para calibrar expectativa de desempenho, não para decidir se o jogo funciona.

## 47. Áudio sob Emscripten (R37.6)

`sounds.mixer_params()` já ramifica em `is_android()` para usar um buffer maior (evita estalo no caminho de áudio do Android, seção 11). Essa função ganha o mesmo ramo para o caso web. O tamanho do buffer entra como um valor de partida, explicitamente marcado como **a calibrar** pela task de spike — registrar um número final sem medir seria repetir o erro que a seção 39 (`Registro honesto`) já evitou para o `buildozer.spec`.

## 48. Entrada: toque, mouse, teclado sob Emscripten (R38.1)

**Hipótese a confirmar, não fato assumido.** O `InputManager` (`input.py`, seção 25) já normaliza teclado/mouse/toque em ações abstratas via eventos SDL (`KEYDOWN`, `MOUSEBUTTONDOWN`, `FINGERDOWN`), com `FINGERMOTION`/`MOUSEMOTION` bloqueados por perf (R27.6). A ponte SDL2 do Emscripten mapeia eventos DOM (clique, toque, tecla) para esses mesmos tipos de evento SDL — se essa ponte se comportar como esperado, nenhuma mudança de código é necessária aqui. A task 88 confirma isso revisitando o spike da task 85; se algo faltar, o ajuste é local a `input.py`.

## 49. Controle Xbox no navegador: Gamepad API (R38.2, R38.3)

O design de hotplug já existente (`JOYDEVICEADDED`/`JOYDEVICEREMOVED`, R10.2) é exatamente a forma certa para o navegador: a Gamepad API do Chrome só revela um controle físico depois de um primeiro gesto do usuário na página (clique, toque ou tecla) — antes disso, `navigator.getGamepads()` simplesmente não lista nada. Isso combina bem com um jogo que já trata o controle como opcional e conectável a qualquer momento, em vez de um requisito de tela de boas-vindas. R38.3 torna explícito que PRONTO nunca pode depender de um botão do controle ter sido pressionado. A task 93 confirma manualmente a paridade de índice de botão (A/Start/Y) entre o mapeamento nativo e o exposto pela Gamepad API.

## 50. Armazenamento via `localStorage` (R39)

**Problema.** `storage.py` resolve hoje um diretório gravável por plataforma (`is_android()`, build congelado do PyInstaller, ou raiz do repositório em desenvolvimento) para `highscore.json`/`quality.json` via `open()`/`json`. No navegador não há sistema de arquivos real acessível dessa forma.

**Solução.** `storage.py` ganha `is_web()` (detecção do ambiente de execução em runtime) e duas funções que substituem o acesso direto a arquivo nos dois módulos que hoje fazem `open()` diretamente:

```python
def is_web() -> bool:
    """Detecta se o jogo roda sob o runtime WebAssembly do pygbag (R39.1)."""
    ...
...
`score.py` e `quality.py` passam a chamar `storage.read_json`/`write_json` em vez de abrir arquivo diretamente — o que também remove a duplicação de tratamento de erro que hoje existe separadamente nos dois módulos, um benefício colateral e não só uma acomodação para o navegador.
...
**Pipeline.** Um novo `scripts/build_web.py` (dev-only, mesma categoria de `scripts/generate_app_icon.py`/`scripts/benchmark.py`) roda em dois passos: primeiro o build padrão do `pygbag` (que produz um `index.html` mais alguns arquivos binários ao lado), depois uma função pura e testável sem `pygbag` real:

```python
def inline_assets(html: str, asset_dir: Path) -> str:
    """Embute em base64 todo asset local referenciado pelo HTML do pygbag,
    substituindo fetch()/<script src> por leitura do proprio documento (R40.1,
    R40.2). Levanta SystemExit se sobrar referencia externa apos o processamento
    (R40.5) - falhar aqui e preferivel a publicar um arquivo que parece unico mas
    ainda depende de rede."""
```

Cada `<script src="...">` local vira um `<script>` inline (já é texto). Cada asset binário local (o pacote de código+assets do `pygbag`, o runtime WASM) vira base64 num `<script type="application/octet-stream;base64" id="asset-...">`, e os pontos do carregador que hoje fazem `fetch()` desses arquivos passam a ler o `<script>` correspondente e decodificar em memória. Uma checagem final varre o HTML resultante por qualquer `fetch(`/`<script src=`/`<link href=` que ainda aponte para um caminho relativo e falha o build (código de saída ≠ 0) se encontrar alguma — é o que impede uma atualização futura do `pygbag` de reintroduzir uma dependência de rede silenciosamente.

**Trade-off de tamanho, registrado explicitamente, não escondido.** O Chrome moderno não tem um limite prático de tamanho para `data:`/arquivo local — dezenas de MB carregam normalmente, só mais devagar em conexão ruim. O custo real não é um limite, é **perder o cache HTTP separado**: o build multi-arquivo padrão do `pygbag` deixa o navegador cachear o runtime WASM (~30 MB) uma vez e reusar entre recarregamentos; um `.html` único faz esse mesmo runtime ser rebaixado a cada visita à URL do GitHub Pages. Em termos de *download*, isso é comparável ao `.exe`/`.apk` de release já existentes; em termos de *visitas repetidas* à mesma URL, é pior. A task 90 mede o número real e devolve a decisão — aceitar como está ou relaxar para multi-arquivo só na URL do Pages — ao dono do produto, não a resolve sozinha.

## 52. CI: GitHub Pages e asset de release (R41)

**`docs.yml` (estendido, não duplicado).** Depois de `mkdocs build --strict` produzir `site/`, um passo novo roda o pipeline de build web e copia o `.html` final para `site/play/index.html`, antes do `actions/upload-pages-artifact` já existente — uma implantação, dois conteúdos publicados juntos (documentação em `/`, jogo em `/play/`).

**`release.yml` (job novo, isolado).** Um job `build-web`, paralelo a `build` (desktop) e `build-apk` (Android), mesmo padrão de isolamento de risco já documentado para o `build-apk` na v2 (seção 24.1: uma falha aqui não deve impedir a publicação dos outros binários). Roda o mesmo `scripts/build_web.py` do `docs.yml` e anexa `BlockyBee-web-<tag>.html` como quarto asset via `softprops/action-gh-release@v2`, igual aos três assets já publicados hoje.

Os dois workflows consomem o mesmo script com a mesma saída — o `.html` hospedado no Pages e o anexado à Release são byte-idênticos, então validar um em Chrome real (task 90) cobre o outro.

## 53. Limites de verificação desta versão

Mesmo espírito da seção 25 (v3, aparelho Android real): o que só um humano no Chrome de verdade confirma, sem caminho automatizável no ambiente de desenvolvimento —

- Qual nível da cascata de renderização (seção 46) de fato vence sob Emscripten, e se o nível acelerado por GPU funciona ali.
- Se o áudio sintetizado toca sem estalo perceptível com o tamanho de buffer escolhido (seção 47).
- Se toque/mouse/teclado têm paridade real com o comportamento nativo (seção 48) — hipótese de zero mudança de código, não fato confirmado até o spike.
- Se o controle Xbox é reconhecido pela Gamepad API depois de um gesto do usuário, com os mesmos índices de botão do caminho nativo (seção 49).
- Se perder o foco da aba durante JOGANDO de fato pausa o jogo, dado que o navegador pode não emitir o mesmo evento SDL usado em desktop/Android (R37.7).
