...
**Escopo da v4:** com o jogo já completo para desktop e Android (v1-v3), a v4 acrescenta uma terceira forma de jogar sem instalar nada: um único arquivo `.html` que roda no Chrome, desktop ou Android, compilado do mesmo código-fonte em `src/` via WebAssembly (`pygbag`), sem fork de lógica de jogo e sem substituir os builds nativos existentes — o `.exe` (PyInstaller) e o `.apk` (Buildozer) continuam sendo publicados normalmente. Quatro frentes: (1) **Execução assíncrona:** o laço principal (`Game.run`) passa a ceder o controle ao navegador a cada quadro, rodando sem alteração tanto sob Emscripten quanto nativamente. (2) **Entrada no navegador:** teclado, mouse, toque e controle Xbox (via Gamepad API do Chrome) continuam funcionando pelo mesmo `InputManager`, sem tela ou passo extra de configuração. (3) **Armazenamento sem sistema de arquivos real:** recorde e nível de qualidade passam a usar `localStorage` quando web, preservando a mesma disciplina de nunca gravar durante uma partida em andamento. (4) **Empacotamento e distribuição:** um script de build embute em base64 tudo que o `pygbag` buscaria por rede, produzindo um `.html` verdadeiramente único, publicado num link estável via GitHub Pages (ao lado do site de documentação já existente) e também anexado como quarto asset de cada Release, ao lado do `.exe` e do `.apk`. Mesmo escopo de dispositivo do v3 (desktop + Android); iOS continua fora — mesmo via navegador, porque no iOS todo app de navegador roda sobre o motor WebKit da Apple, não sobre o Chromium real. Nenhum requisito de v1-v3 é removido.

**Escopo da v3 (mantido):** além de tudo que a v1 e a v2 entregaram (jogo completo para desktop e para Android, ver `specs/v1/` e `specs/v2/`), a v3 ataca seis frentes. (1) **Identidade:** o jogo passa a se chamar **Blocky Bee** — o personagem sempre foi uma abelha, e o nome antigo ("Bird") era herança do gênero, não do jogo. (2) **Aproveitamento de tela:** a barra preta que sobrava em todo aparelho — nos quatro lados no Android, nas laterais num monitor maximizado — é eliminada em todos os sistemas operacionais, sem cortar a cena e **sem alterar a dificuldade**: a área jogável continua sendo exatamente a mesma coluna de 480×720 de mundo, e o espaço que sobra vira faixa decorativa com temática Minecraft (céu estendido, terra mais funda, corte transversal do subsolo e mobs que não interagem com o jogo). A tela inteira também passa a valer como superfície de entrada: tocar sobre a faixa decorativa faz o pássaro voar como se o toque tivesse caído na área jogável, para que nenhuma parte da tela do celular seja zona morta. (3) **Desempenho:** o jogo passa a ser renderizado com aceleração por GPU por padrão, com todo o conteúdo estático pré-renderizado, pressão de coletor de lixo reduzida e taxa de quadros independente do aparelho — o Android da v2 sofria com queda de FPS e com física em câmera lenta quando não sustentava 60 FPS. (4) **Documentação:** toda classe e função pública passa a ter docstring obrigatória, verificada em CI, e o projeto ganha um site de documentação que publica a API junto com os documentos de spec. (5) **Rigor de SDD:** matriz de rastreabilidade ligando cada critério de aceitação ao design, à tarefa e ao teste que o prova, verificada por teste automatizado, mais cobertura mínima no CI, e um README que explica a motivação do projeto e o método. (6) **Boas práticas de Git e GitHub:** as convenções de branch, commit e tag já em uso informalmente passam a estar documentadas, com `LICENSE`, hooks de pre-commit espelhando o gate de CI, Dependabot e demais itens de higiene esperados de um projeto Python público. O desktop continua suportado; nenhum requisito da v1 ou da v2 é removido.
...
## R37 — Compatibilidade com navegador via WebAssembly (pygbag)

**User story:** Como jogador sem instalar nada, quero abrir o Blocky Bee no Chrome (desktop ou Android) e jogar a versão completa, exatamente como nas versões nativas.

### Critérios de aceitação

1. O jogo DEVE ser compilado para WebAssembly a partir do mesmo código-fonte em `src/`/`main.py` usado nos builds nativos, via `pygbag`, sem fork de lógica de jogo nem laço principal duplicado.
2. O laço principal (`Game.run`) DEVE ser assíncrono (`async def`, executado via `asyncio.run`), cedendo o controle ao navegador a cada quadro com `await asyncio.sleep(0)`.
3. O MESMO código do laço principal DEVE continuar rodando sem alteração no desktop e no Android nativos — nenhum ramo de plataforma condicionando o laço em si.
4. QUANDO o jogo abre em Chrome atualizado (desktop ou Android) a partir do arquivo publicado, O sistema DEVE alcançar o estado PRONTO e responder a comandos, com física, biomas, colisão e pontuação idênticos às versões nativas.
...
## R38 — Entrada no navegador (toque, mouse, teclado, controle Xbox)

**User story:** Como jogador no Chrome, quero usar o mesmo controle que uso nativamente, sem tela ou botão extra de "modo navegador".

### Critérios de aceitação

1. Teclado, mouse e toque DEVEM continuar funcionando através do mesmo `InputManager` sem alteração de código, reafirmando R1.1, R15.1 e R34.1 para o navegador.
2. QUANDO um controle Xbox está conectado e exposto pela Gamepad API do navegador, O sistema DEVE reconhecê-lo pelo mesmo caminho de hotplug já existente (`JOYDEVICEADDED`, R10.2), sem tela própria de "pressione um botão para conectar".
3. RECONHECENDO que a Gamepad API do Chrome só revela o controle depois de um primeiro gesto do usuário na página, ANTES desse gesto os botões do controle NÃO DEVEM ser exigidos para chegar a PRONTO — teclado, mouse ou toque bastam.
4. Nenhuma tecla ou gesto exclusivo do navegador, além dos já listados no filtro de eventos existente (R27.6), DEVE ser capturado pelo jogo.

---

## R39 — Armazenamento em navegador (recorde e qualidade)

**User story:** Como jogador no Chrome, quero que meu recorde sobreviva a recarregar a página, mesmo sem sistema de arquivos de verdade.

### Critérios de aceitação

1. `storage.py` DEVE expor uma função `is_web()` (análoga a `is_android()`), detectando o ambiente de execução em runtime.
2. QUANDO `is_web()` é verdadeiro, a leitura e a gravação de `highscore.json` e de `quality.json` (R4.3-R4.5, R29.5) DEVEM usar armazenamento do navegador em vez de arquivo, preservando a mesma API pública já usada por `score.py`/`quality.py`.
3. A ausência ou corrupção do dado salvo no navegador NÃO DEVE impedir o jogo de abrir — mesma degradação graciosa já garantida para arquivo ausente/corrompido em desktop/Android.
4. A gravação no navegador DEVE respeitar a mesma disciplina de R27.5 — nunca durante JOGANDO, só nos mesmos pontos de flush já definidos — sem ramo de código novo em `game.py` para o caso web.
5. Os testes automatizados DEVEM exercitar o caminho web sem navegador real, via um backend de armazenamento falso injetável, cobrindo os mesmos casos de dado ausente/corrompido já testados para desktop/Android.

---

## R40 — Empacotamento Web como arquivo `.html` único

**User story:** Como jogador, quero baixar (ou abrir via link) um único arquivo `.html` que contém o jogo inteiro, sem depender de outros arquivos ao lado nem de acesso à internet depois de carregado.

### Critérios de aceitação

1. O build Web DEVE produzir um único arquivo (`BlockyBee.html` / `BlockyBee-web-<tag>.html`) que, aberto num navegador (local ou hospedado), executa o jogo completo sem depender de nenhum outro arquivo carregado por URL relativa própria do jogo.
2. O processo de build DEVE ser reprodutível a partir do código-fonte por um único comando de alto nível, documentado, que gera o `.html` final a partir de `pygbag` mais um passo de pós-processamento.
3. QUANDO o arquivo é aberto num Chrome com as ferramentas de rede abertas, NENHUMA requisição de rede adicional DEVE ocorrer além do carregamento do próprio arquivo.
4. O renderizador acelerado por GPU (R26, R37.5) DEVE ser buildado com a configuração de contexto exigida pelo `pygame._sdl2.video.Renderer` usado nos níveis 1 e 2 da cascata.
5. O processo de build DEVE falhar (código de saída diferente de zero) SE, após o pós-processamento, restar qualquer referência a arquivo local externo no `.html` final.

---

## R41 — Distribuição Web (GitHub Pages + asset de release)

**User story:** Como jogador, quero um link estável para jogar direto no navegador, e também poder baixar o mesmo jogo como arquivo, igual já acontece hoje com o `.exe` e o `.apk`.

### Critérios de aceitação

1. QUANDO uma alteração da v4 chega à branch `main`, O CI DEVE (re)construir o arquivo `.html` único e publicá-lo em GitHub Pages, na MESMA implantação já usada pelo site de documentação (R31.4) — sem que as duas publicações se sobrescrevam.
2. QUANDO uma Release é publicada com tag de versão (mesmo gatilho de R13.2/R17.2), O CI DEVE anexar o MESMO arquivo `.html` como quarto asset (`BlockyBee-web-<tag>.html`), gerado pelo mesmo processo usado para o GitHub Pages.
3. O link do GitHub Pages DEVE continuar funcionando na mesma URL entre publicações.
4. Nenhum requisito de R1-R36 DEVE deixar de valer no navegador, exceto onde a seção de verificação manual desta versão documentar uma limitação conhecida e aceita.

---

## Fora de escopo

Multiplayer, skins alternativas, música de fundo contínua, menus de configuração, salvamento em nuvem, publicação na Google Play Store (o APK é para instalação direta; assinatura de release com keystore próprio fica para uma versão futura), suporte a iOS — **inclusive via navegador**: no iOS todo app de navegador, mesmo o "Chrome", roda sobre o motor WebKit da Apple, não sobre o Chromium/V8 real, então não dá acesso ao mesmo potencial que a v4 busca. Ícone temático/monocromático do Android 13+ ("Material You" themed icons) e imagem de destaque ("feature graphic") de loja de aplicativos ficam fora de escopo — dependem de publicação na Play Store, já fora de escopo. Scaffolding de projeto open-source voltado a múltiplos contribuidores externos — templates de issue, `CODE_OF_CONDUCT.md`, `SECURITY.md`, `CODEOWNERS` — também fica fora: este é um projeto pessoal/pai-e-filho, sem processo formal de triagem de issues de terceiros. Suporte offline via PWA/service worker e cache separado do runtime Web (que reduziria o download em visitas repetidas, às custas de o `.html` deixar de ser literalmente autocontido) ficam fora da v4 — podem entrar numa versão futura se o tamanho do arquivo único se mostrar um problema real na prática (ver design.md seção 51).
