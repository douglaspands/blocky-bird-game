...
**Escopo da v3:** dois itens de qualidade que não mudam o gameplay. (1) Todo o código Python do projeto passa a ser verificado pela ferramenta `ruff` (lint + formatação), com conformidade obrigatória e checada em CI. (2) A fonte bitmap própria (v2, R7.6) está grande demais em várias telas — textos se sobrepõem entre si ou com outros elementos de UI — e o tamanho ideal por tela precisa ser levantado e corrigido. Nenhum requisito das versões anteriores é removido.
