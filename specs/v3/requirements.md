...
**Escopo da v3:** o jogo ainda não tem ícone próprio — nem na janela/executável do desktop nem no APK Android, que usam o ícone genérico do framework. A v3 adiciona um ícone do aplicativo com a personagem do jogo (a abelha voxel, R7.2), gerado por código como todo o resto da apresentação visual (R7.1), em todas as plataformas já suportadas: janela e executável do desktop (Windows/Linux, R13), e ícone do launcher no Android — incluindo o formato de **ícone adaptativo** exigido desde o Android 8.0 (API 26), para que a abelha não fique cortada ou distorcida pelas diferentes máscaras de ícone dos fabricantes (círculo, "squircle", quadrado arredondado). Nenhum requisito das versões anteriores é removido.
